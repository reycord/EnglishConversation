...
class level_model extends My_Model
{
    ...
    public function getLevelNameVocabulary()
    {
    ...
    // get level name from table m_level
    public function getLevelNameListening()
    {
        $this->db->select('m_level.level_id as level_id');
        $this->db->select('m_level.level_name as level_name');
        $this->db->select('m_level.level_image as level_image');
        $this->db->select('count(tbl_listening.level_id) as total');
        $this->db->from('m_level');
        $this->db->join('tbl_listening', 'm_level.level_id = tbl_listening.level_id', 'left');
        $this->db->group_by('m_level.level_id');
        $this->db->order_by('m_level.level_id');
        $query = $this->db->get();
        $result = $query->result_array();
    ...
    // get level name from table m_level
    public function getLevelNameGrammar()
    {
        $this->db->select('m_level.level_id as level_id');
        $this->db->select('m_level.level_name as level_name');
        $this->db->select('m_level.level_image as level_image');
        $this->db->select('count(tbl_grammar.level_id) as total');
        $this->db->from('m_level');
        $this->db->join('tbl_grammar', 'm_level.level_id = tbl_grammar.level_id', 'left');
        $this->db->group_by('m_level.level_id');
        $this->db->order_by('m_level.level_id');
        $query = $this->db->get();
        $result = $query->result_array();
